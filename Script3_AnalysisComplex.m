...
RT = data_passation.responsetime(1:length(n_response));

CIlim = [0.25 97.5];

%% Behaviour 

clear i_m N_m m_bin m_edge idx_m H M CR FA

n_window = 100;
minNformean = 50;

[N_m,m_edge] = histcounts(m, [-15.5:1:-0.5]);
...
subplot(2,2,1); plot(1:length(m), m, 'g'); hold on; plot(1:n_window:length(m), m_windowed, 'k'); ylim([m_edge(1) m_edge(end)]); xlabel(' trial #'); ylabel('m (dB)'); xlim([1 length(m)]); ylimits=ylim;
for i = 1:length(data_passation.resume_trial)
    plot(data_passation.resume_trial(i)*[1 1],ylimits,'k:');
end
subplot(2,2,3); plot(1:n_window:length(m),PC_targetpresent); hold on; plot(1:n_window:length(m),PC_targetabsent);  xlim([1 length(m)]); xlabel(' trial #'); ylabel('correct response rate'); ylim([0 1]); hold on; plot([1 length(m)],[0.5 0.5],'k--'); ylimits=ylim;
for i = 1:length(data_passation.resume_trial)
    plot(data_passation.resume_trial(i)*[1 1],ylimits,'k:');
end
subplot(2,2,2); bar(m_bin, [H', M', CR', FA']); xlim([m_edge(1) m_edge(end)]); xlabel('m (dB)'); ylabel('Nb of trials'); hold on; plot([m_edge(1) m_edge(end)],[minNformean minNformean]/2,'k:');legend({'H', 'M', 'CR', 'FA', 'Nmin'});
subplot(2,2,4); bar(m_bin, [H'./(M'+H'), CR'./(CR'+FA')].*[M'+H'>minNformean,CR'+FA'>minNformean]);xlim([m_edge(1) m_edge(end)]); xlabel('m (dB)'); ylabel('correct response rate'); hold on; plot([[m_edge(1) m_edge(end)]],[0.5 0.5],'k--'); legend({'target present', 'target absent','chance level'},'Location','southeast');

saveas(gcf,'Behavior.png')
