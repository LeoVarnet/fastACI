function fAudika_run_sessions(subjectname,Contrast,bCheck_dropbox)
% function fAudika_run_sessions(modelname,Contrast,bCheck_dropbox)
%
...
if nargin == 0
    subjectname = input('Enter the ID of the subject to be tested (e.g., ''HI09''): '); % 'SAO'; %'SAO'; % modelname = 'king2019';
end

HT_path = [fastACI_paths('dir_data') 'HearingThresholds' filesep];
fileID = fopen([HT_path subjectname '.txt'],'r'); 
if fileID==-1
    error(['Missing Audiogram! File ' HT_path subjectname '.txt not found']);
end
dataHL2 = fscanf(fileID,'%d %d %d %d %d %d %d %d %d', [9,3]); dataHL2=dataHL2';
if size(dataHL2,1) == 2
    HL2 = dataHL2;
    HLF = [250 500 750 1000 1500 2000 4000 6000 8000];
elseif size(dataHL2,1) == 3
    HL2 = dataHL2(2:3,:);
    HLF = dataHL2(1,:);
    if ~any(HLF>1000)
        error('incorrect frequency vector detected in Hearing Thresholds\n')
    end
else
    error('incorrect size for Hearing Thresholds\n')
end
fclose(fileID);

Contrasts2test = {'abda','adga','abpa','adta','apta','agka','akta'};
if nargin < 2
...
experiment = ['speechACI_Audika-' Contrast '-S43M']; 
...
% experiment = 'speechACI_Audika-abda-S43M';
