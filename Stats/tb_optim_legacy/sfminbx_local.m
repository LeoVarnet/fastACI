function [x,FVAL,LAMBDA,EXITFLAG,OUTPUT,GRAD,HESSIAN] = sfminbx_local(funfcn,x,l,u,verb, ...
...
%SFMINBX_LOCAL Nonlinear minimization with box constraints.
%
...
optionFeedback = il_createOptionFeedback(options);
   ...
   [xOutputfcn, optimValues, stop] = callOutputAndPlotFcns(outputfcn,plotfcns,xcurr,xOutputfcn,'init',iter,numFunEvals, ...
    ...
    if stop
        [x,FVAL,LAMBDA,EXITFLAG,OUTPUT,GRAD,HESSIAN] = ...
            il_cleanUpInterrupt(xOutputfcn,optimValues,npcg,verb,detailedExitMsg,algorithmName,caller);
        return;
    end
end
...
while ~done
    ...
    if ((gnrm < tol1) && (posdef == 1) )
        ...
        outMessage = createExitMsg_local('sfminle',msgFlag,verb > 1,detailedExitMsg,caller, ...
            gnrm,optionFeedback.TolFun,tol1);
    elseif (nrmsx < .9*delta) && (ratio > .25) && (diff < tol1*(1+abs(oval)))
        done = true; 
        EXITFLAG = 3;
        outMessage = createExitMsg_local('sfminle',EXITFLAG,verb > 1,detailedExitMsg,caller, ...
    ...
    elseif numFunEvals > maxfunevals
        ...
        EXITFLAG = 0;
        outMessage = createExitMsg_local('sfminle',EXITFLAG,verb > 0,detailedExitMsg,caller, ...
            [],optionFeedback.MaxFunEvals,maxfunevals);
    elseif iter > maxiter
        done = true; 
        EXITFLAG = 0;
        % Call createExitMsg with createExitMsgExitflag = 10 for MaxIter exceeded
        outMessage = createExitMsg_local('sfminle',10,verb > 0,detailedExitMsg,caller, ...
...
%--------------------------------------------------------------------------
function [x,FVAL,LAMBDA,EXITFLAG,OUTPUT,GRAD,HESSIAN] = il_cleanUpInterrupt(xOutputfcn,optimValues,npcg, ...
...
OUTPUT.message = createExitMsg_local('sfminle',EXITFLAG,verb > 0,detailedExitMsg,caller);
...
%--------------------------------------------------------------------------
function optionFeedback = il_createOptionFeedback(options)
...
%   $Revision: 1.1.6.4 $  $Date: 2011/05/09 01:16:01 $
