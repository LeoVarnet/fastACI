function [outsig, fc, mfc, step] = king2019a(insig,fs,varargin)
%KING2019A Modulation filterbank (based on nonlinear processing)
%   
%   Usage:    outsig = king2019a(insig,fs,basef)
%             [outsig, fc, mfc, step] = king2019a(insig,fs,varargin)
%              [outsig, fc, mfc, step]= king2019a(insig,fs,flow,fhigh,parameters)
% 
%   Input parameter:
...
%
%   `king2019a' is based on `king2019' with the only difference that
%   internal noise is added to the internal representations.
%
%   `king2019a(insig,fs,'basef',basef)` computes the internal representation 
%   around the frequency *basef* of the signal *insig* sampled with 
%   a frequency of *fs* Hz. *outsig* is a matrix of size [length fc mfc]. 
%  
%   `[outsig,fc,mfc,step]=king2019a(...)` additionally returns the 
...
%
%     6. Additive internal noise, multiplicative internal noise, and 
%     memory internal noise.
%
...
%   #Author: Alejandro Osses (2023) Including the different internal noises
...
definput.import={'auditoryfilterbank','ihcenvelope','king2019','king2019a'}; % load from arg_king2019
...
%%% New in king2019a (to be integrated in AMT)
% Initialising a scaling variable:
if flags.do_internalnoiseadd || flags.do_internalnoisemem
    gain2use = 10^((3-kv.dboffset)/20); % attenuation of 97 dB if the AMT standard is used
end
% First, the additive and multiplicative are created:
if flags.do_internalnoiseadd
    intnoiseadd_std = gain2use*kv.intnoiseadd_std;
    
    addnoise = wallaert2017_internalnoiseadd(outsig,subfs,intnoiseadd_std);
end
if flags.do_internalnoisemult
    multnoise = wallaert2017_internalnoisemult(outsig,subfs,kv.intnoisemult_ratio_dB);
end

% Second, the additive and multiplicative are added:
if flags.do_internalnoiseadd
    outsig = outsig + addnoise;
end
if flags.do_internalnoisemult
    outsig = outsig + multnoise;
end

% Third, the memory noise is created and added to the already decremented 
%   internal representations.
if flags.do_internalnoisemem
    intnoisemem_std = gain2use*kv.intnoisemem_std;
    % This noise is added directly:
    memnoise = wallaert2017_internalnoisemem(outsig,subfs,intnoisemem_std,kv.intnoisemem_tau);
    outsig = outsig + memnoise;
end
step.subfs = subfs;
